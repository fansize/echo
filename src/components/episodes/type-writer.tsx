...
export default function TypeWriter({ targetContent: targetText }: Props) {
    ...
    const words = targetText.replace(/[.,\/#!$%\^&\*;:{}=\-_`~()]/g, '').split(' ');
    ...
    useEffect(() => {
        const handleKeyDown = (event: KeyboardEvent) => {
            ...
            if (wordIndex >= words.length || userInput.length >= words[wordIndex].length) {
                setUserInput('');
                setCorrectInput('');
                setWordIndex(0);
                return;
                    ...
                    <>
                        {word.split('').map((char, charIdx) => {
                            ...
                            if (wordIdx < wordIndex) {
                                ...
                                color = charIdx < correctInput.length ? 'green' : (charIdx < userInput.length ? 'red' : 'black');
                            }
                            return <span key={`${wordIdx}-${charIdx}`} style={{ color }}>{char}</span>;
                        })}
